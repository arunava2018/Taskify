    ...
    <section className="lg:py-5 py-3 bg-[var(--background)] text-[var(--foreground)]">
        ...
        <div className="text-center mb-10 md:max-w-3xl max-w-2xl mx-auto p-2">
          <h2 className="text-5xl md:text-5xl lg:text-7xl font-bold mb-4">
            Loved by teams <span className="text-[var(--primary)]">everywhere</span>
          </h2>
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {TESTIMONIALS.map(({ id, name, role, company, feedback, avatar, rating }) => (
            <div
              key={id}
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-sm font-semibold text-[var(--card-foreground)] truncate">{name}</h4>
                  <p className="text-xs text-[var(--muted-foreground)] truncate">{role}</p>
                  {company && (
                    <p className="text-xs text-[var(--muted-foreground)] truncate">{company}</p>
                  )}
                </div>
              </div>
            </div>
